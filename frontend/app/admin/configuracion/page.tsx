...
export default function ConfiguracionPage() {
  ...
  return (
    <div className="space-y-5 animate-fadeIn max-w-2xl">
      <div>
        <h1 className="text-2xl font-bold text-[#F2F2F2]">Configuración</h1>
        <p className="text-sm text-[#8A8A8A] mt-0.5">Ajustes de tu cuenta y la plataforma</p>
      </div>
      ...
      <Section title="Mi cuenta" icon={<User size={16} />}>
        <div className="space-y-4">
          ...
          <button onClick={handleSaveAccount}
            className={`flex items-center gap-2 px-4 py-2 text-sm rounded-xl font-medium transition-all cursor-pointer ${saved ? 'bg-[#00E5A0]/15 text-[#00E5A0] border border-[#00E5A0]/30' : 'bg-[#FF7A1A] text-white hover:bg-[#E0650A]'}`}>
      ...
      <Section title="Cambiar contraseña" icon={<Lock size={16} />}>
        <div className="space-y-4">
              ...
              <div className="relative">
                ...
                {i === 0 && (
                  <button type="button" onClick={() => setShowPass(s => !s)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-[#8A8A8A] hover:text-[#F2F2F2] cursor-pointer">
          ...
          <button className="flex items-center gap-2 px-4 py-2 text-sm rounded-xl bg-[#FF7A1A] text-white font-medium hover:bg-[#E0650A] transition-colors cursor-pointer">
      ...
      <Section title="Desvío a atención humana" icon={<PhoneCall size={16} />}>
        ...
        <Field label="Número de WhatsApp personal">
          <div className="flex gap-3">
            ...
            <button className="flex items-center gap-2 px-4 py-2 text-sm rounded-xl bg-[#FF7A1A] text-white font-medium hover:bg-[#E0650A] transition-colors cursor-pointer whitespace-nowrap">
      ...
      <Section title="Planes disponibles" icon={<Tag size={16} />}>
        <p className="text-xs text-[#8A8A8A] mb-4">Edita los precios de los planes. Los cambios aplican a nuevas suscripciones.</p>
        <div className="space-y-3">
          {plans.map(plan => (
            <div key={plan.id} className="flex items-center justify-between p-4 rounded-xl border border-[#2A2A2A] bg-[#1C1C1C]">
              <div className="flex-1">
                <p className="text-sm font-semibold text-[#F2F2F2]">{plan.name}</p>
                <p className="text-xs text-[#8A8A8A] mt-0.5">{plan.desc}</p>
                <p className="text-xs text-[#8A8A8A] mt-0.5">{plan.msgs === -1 ? 'Mensajes ilimitados' : `${plan.msgs.toLocaleString()} msgs/día`}</p>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-xs text-[#8A8A8A]">S/</span>
                <input
                  type="number"
                  value={plan.price}
                ...
                />
                <span className="text-xs text-[#8A8A8A]">/mes</span>
              </div>
            </div>
          ))}
        </div>
        <button className="mt-4 flex items-center gap-2 px-4 py-2 text-sm rounded-xl bg-[#FF7A1A] text-white font-medium hover:bg-[#E0650A] transition-colors cursor-pointer">
